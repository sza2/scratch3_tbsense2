...
class TbSense2 {
    ...
    _onMessage (base64) {
        ...
        // the temperature value is in milli-Celsius
        this._sensors.temperature = (data[6] << 8 | (data[7])) / 1000.0;

        // the temperature value is in milli-percent
        this._sensors.humidity = (data[8] << 8 | (data[9])) / 1000.0;
...
class Scratch3TbSense2Blocks {
    ...
    get TEMPERATURE_UNIT_MENU () {
        return [
            {
                text: formatMessage({
                    id: 'TbSense2.temperatureUnitMenu.celsius',
                    default: 'celsius',
                    description: 'label for celsius element in temperature unit picker for TB Sense 2 extension'
                }),
                value: TbSense2TemperatureUnit.CELSIUS
            },
            {
                text: formatMessage({
                    id: 'TbSense2.temperatureUnitMenu.fahrenheit',
                    default: 'fahrenheit',
                    description: 'label for fahrenheit element in temperature unit picker for TB Sense 2 extension'
                }),
                value: TbSense2TemperatureUnit.FAHRENHEIT
            },
            {
                text: formatMessage({
                    id: 'TbSense2.temperatureUnitMenu.kelvin',
                    default: 'kelvin',
                    description: 'label for kelvin element in temperature unit picker for TB Sense 2 extension'
                }),
                value: TbSense2TemperatureUnit.KELVIN
            }
       ];
    }
        ...
        return {
            ...
            blocks: [
                ...
                '---',
                {
                    ...
                    text: formatMessage({
                        id: 'TbSense2.temperature',
                        default: 'temperature in [UNIT]',
                        description: 'temperature from the TB Sense 2 Si7021 sensor'
                    }),
                    blockType: BlockType.REPORTER,
                    arguments: {
                        UNIT: {
                            type: ArgumentType.STRING,
                            menu: 'temperatureUnit',
                            defaultValue: TbSense2TemperatureUnit.CELSIUS
                        }
                    }
                },
                {
                    opcode: 'getHumidity',
                    text: formatMessage({
                        id: 'TbSense2.humidity',
                        default: 'humidity',
                        description: 'humidity from the TB Sense 2 Si7021 sensor'
                    }),
                    blockType: BlockType.REPORTER,
                },
                '---',
    ...
    /**
     * @param {object} args - the block's arguments.
     * @return {number} - the RHT sensor's temperature value.
     */
    getTemperature (args) {
        return this._getTemperature(args.UNIT);
    }

    /**
     * @param {TemperatureUnit} unit - Celsius, Fahrenheit or Kelvin.
     * @return {number} - the temperature value from the RHT sensor.
     * @private
     */
    _getTemperature (unit) {
        switch (unit) {
        case TbSense2TemperatureUnit.CELSIUS:
            return this._peripheral.temperature.toFixed(1);
        case TbSense2TemperatureUnit.FAHRENHEIT:
            return (this._peripheral.temperature * 9.0 / 5.0 + 32).toFixed(1);
        case TbSense2TemperatureUnit.KELVIN:
            return (this._peripheral.temperature + 273.15).toFixed(1);
        default:
            log.warn(`Unknown temperature unit in _getTemperature: ${unit}`);
        }
    }

    /**
     * @param {object} args - the block's arguments.
     * @return {number} - the RHT sensor's humidity value.
     */
    getHumidity (args) {
        return this._getHumidity();
    }

    /**
     * @param {}
     * @return {number} - the relative humidity from the RHT sensor.
     * @private
     */
    _getHumidity () {
        return this._peripheral.humidity.toFixed(1);
    }
